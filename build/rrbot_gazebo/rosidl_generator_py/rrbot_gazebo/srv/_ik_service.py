# generated from rosidl_generator_py/resource/_idl.py.em
# with input from rrbot_gazebo:srv/IKService.idl
# generated code does not contain a copyright notice


# Import statements for member types

import builtins  # noqa: E402, I100

import math  # noqa: E402, I100

import rosidl_parser.definition  # noqa: E402, I100


class Metaclass_IKService_Request(type):
    """Metaclass of message 'IKService_Request'."""

    _CREATE_ROS_MESSAGE = None
    _CONVERT_FROM_PY = None
    _CONVERT_TO_PY = None
    _DESTROY_ROS_MESSAGE = None
    _TYPE_SUPPORT = None

    __constants = {
    }

    @classmethod
    def __import_type_support__(cls):
        try:
            from rosidl_generator_py import import_type_support
            module = import_type_support('rrbot_gazebo')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'rrbot_gazebo.srv.IKService_Request')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            cls._CREATE_ROS_MESSAGE = module.create_ros_message_msg__srv__ik_service__request
            cls._CONVERT_FROM_PY = module.convert_from_py_msg__srv__ik_service__request
            cls._CONVERT_TO_PY = module.convert_to_py_msg__srv__ik_service__request
            cls._TYPE_SUPPORT = module.type_support_msg__srv__ik_service__request
            cls._DESTROY_ROS_MESSAGE = module.destroy_ros_message_msg__srv__ik_service__request

    @classmethod
    def __prepare__(cls, name, bases, **kwargs):
        # list constant names here so that they appear in the help text of
        # the message class under "Data and other attributes defined here:"
        # as well as populate each message instance
        return {
        }


class IKService_Request(metaclass=Metaclass_IKService_Request):
    """Message class 'IKService_Request'."""

    __slots__ = [
        '_input_x',
        '_input_y',
        '_input_z',
    ]

    _fields_and_field_types = {
        'input_x': 'float',
        'input_y': 'float',
        'input_z': 'float',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.BasicType('float'),  # noqa: E501
        rosidl_parser.definition.BasicType('float'),  # noqa: E501
        rosidl_parser.definition.BasicType('float'),  # noqa: E501
    )

    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        self.input_x = kwargs.get('input_x', float())
        self.input_y = kwargs.get('input_y', float())
        self.input_z = kwargs.get('input_z', float())

    def __repr__(self):
        typename = self.__class__.__module__.split('.')
        typename.pop()
        typename.append(self.__class__.__name__)
        args = []
        for s, t in zip(self.__slots__, self.SLOT_TYPES):
            field = getattr(self, s)
            fieldstr = repr(field)
            # We use Python array type for fields that can be directly stored
            # in them, and "normal" sequences for everything else.  If it is
            # a type that we store in an array, strip off the 'array' portion.
            if (
                isinstance(t, rosidl_parser.definition.AbstractSequence) and
                isinstance(t.value_type, rosidl_parser.definition.BasicType) and
                t.value_type.typename in ['float', 'double', 'int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32', 'int64', 'uint64']
            ):
                if len(field) == 0:
                    fieldstr = '[]'
                else:
                    assert fieldstr.startswith('array(')
                    prefix = "array('X', "
                    suffix = ')'
                    fieldstr = fieldstr[len(prefix):-len(suffix)]
            args.append(s[1:] + '=' + fieldstr)
        return '%s(%s)' % ('.'.join(typename), ', '.join(args))

    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.input_x != other.input_x:
            return False
        if self.input_y != other.input_y:
            return False
        if self.input_z != other.input_z:
            return False
        return True

    @classmethod
    def get_fields_and_field_types(cls):
        from copy import copy
        return copy(cls._fields_and_field_types)

    @builtins.property
    def input_x(self):
        """Message field 'input_x'."""
        return self._input_x

    @input_x.setter
    def input_x(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'input_x' field must be of type 'float'"
            assert not (value < -3.402823466e+38 or value > 3.402823466e+38) or math.isinf(value), \
                "The 'input_x' field must be a float in [-3.402823466e+38, 3.402823466e+38]"
        self._input_x = value

    @builtins.property
    def input_y(self):
        """Message field 'input_y'."""
        return self._input_y

    @input_y.setter
    def input_y(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'input_y' field must be of type 'float'"
            assert not (value < -3.402823466e+38 or value > 3.402823466e+38) or math.isinf(value), \
                "The 'input_y' field must be a float in [-3.402823466e+38, 3.402823466e+38]"
        self._input_y = value

    @builtins.property
    def input_z(self):
        """Message field 'input_z'."""
        return self._input_z

    @input_z.setter
    def input_z(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'input_z' field must be of type 'float'"
            assert not (value < -3.402823466e+38 or value > 3.402823466e+38) or math.isinf(value), \
                "The 'input_z' field must be a float in [-3.402823466e+38, 3.402823466e+38]"
        self._input_z = value


# Import statements for member types

# already imported above
# import builtins

# already imported above
# import math

# already imported above
# import rosidl_parser.definition


class Metaclass_IKService_Response(type):
    """Metaclass of message 'IKService_Response'."""

    _CREATE_ROS_MESSAGE = None
    _CONVERT_FROM_PY = None
    _CONVERT_TO_PY = None
    _DESTROY_ROS_MESSAGE = None
    _TYPE_SUPPORT = None

    __constants = {
    }

    @classmethod
    def __import_type_support__(cls):
        try:
            from rosidl_generator_py import import_type_support
            module = import_type_support('rrbot_gazebo')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'rrbot_gazebo.srv.IKService_Response')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            cls._CREATE_ROS_MESSAGE = module.create_ros_message_msg__srv__ik_service__response
            cls._CONVERT_FROM_PY = module.convert_from_py_msg__srv__ik_service__response
            cls._CONVERT_TO_PY = module.convert_to_py_msg__srv__ik_service__response
            cls._TYPE_SUPPORT = module.type_support_msg__srv__ik_service__response
            cls._DESTROY_ROS_MESSAGE = module.destroy_ros_message_msg__srv__ik_service__response

    @classmethod
    def __prepare__(cls, name, bases, **kwargs):
        # list constant names here so that they appear in the help text of
        # the message class under "Data and other attributes defined here:"
        # as well as populate each message instance
        return {
        }


class IKService_Response(metaclass=Metaclass_IKService_Response):
    """Message class 'IKService_Response'."""

    __slots__ = [
        '_joint_x',
        '_joint_y',
        '_joint_z',
    ]

    _fields_and_field_types = {
        'joint_x': 'float',
        'joint_y': 'float',
        'joint_z': 'float',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.BasicType('float'),  # noqa: E501
        rosidl_parser.definition.BasicType('float'),  # noqa: E501
        rosidl_parser.definition.BasicType('float'),  # noqa: E501
    )

    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        self.joint_x = kwargs.get('joint_x', float())
        self.joint_y = kwargs.get('joint_y', float())
        self.joint_z = kwargs.get('joint_z', float())

    def __repr__(self):
        typename = self.__class__.__module__.split('.')
        typename.pop()
        typename.append(self.__class__.__name__)
        args = []
        for s, t in zip(self.__slots__, self.SLOT_TYPES):
            field = getattr(self, s)
            fieldstr = repr(field)
            # We use Python array type for fields that can be directly stored
            # in them, and "normal" sequences for everything else.  If it is
            # a type that we store in an array, strip off the 'array' portion.
            if (
                isinstance(t, rosidl_parser.definition.AbstractSequence) and
                isinstance(t.value_type, rosidl_parser.definition.BasicType) and
                t.value_type.typename in ['float', 'double', 'int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32', 'int64', 'uint64']
            ):
                if len(field) == 0:
                    fieldstr = '[]'
                else:
                    assert fieldstr.startswith('array(')
                    prefix = "array('X', "
                    suffix = ')'
                    fieldstr = fieldstr[len(prefix):-len(suffix)]
            args.append(s[1:] + '=' + fieldstr)
        return '%s(%s)' % ('.'.join(typename), ', '.join(args))

    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.joint_x != other.joint_x:
            return False
        if self.joint_y != other.joint_y:
            return False
        if self.joint_z != other.joint_z:
            return False
        return True

    @classmethod
    def get_fields_and_field_types(cls):
        from copy import copy
        return copy(cls._fields_and_field_types)

    @builtins.property
    def joint_x(self):
        """Message field 'joint_x'."""
        return self._joint_x

    @joint_x.setter
    def joint_x(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'joint_x' field must be of type 'float'"
            assert not (value < -3.402823466e+38 or value > 3.402823466e+38) or math.isinf(value), \
                "The 'joint_x' field must be a float in [-3.402823466e+38, 3.402823466e+38]"
        self._joint_x = value

    @builtins.property
    def joint_y(self):
        """Message field 'joint_y'."""
        return self._joint_y

    @joint_y.setter
    def joint_y(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'joint_y' field must be of type 'float'"
            assert not (value < -3.402823466e+38 or value > 3.402823466e+38) or math.isinf(value), \
                "The 'joint_y' field must be a float in [-3.402823466e+38, 3.402823466e+38]"
        self._joint_y = value

    @builtins.property
    def joint_z(self):
        """Message field 'joint_z'."""
        return self._joint_z

    @joint_z.setter
    def joint_z(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'joint_z' field must be of type 'float'"
            assert not (value < -3.402823466e+38 or value > 3.402823466e+38) or math.isinf(value), \
                "The 'joint_z' field must be a float in [-3.402823466e+38, 3.402823466e+38]"
        self._joint_z = value


class Metaclass_IKService(type):
    """Metaclass of service 'IKService'."""

    _TYPE_SUPPORT = None

    @classmethod
    def __import_type_support__(cls):
        try:
            from rosidl_generator_py import import_type_support
            module = import_type_support('rrbot_gazebo')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'rrbot_gazebo.srv.IKService')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            cls._TYPE_SUPPORT = module.type_support_srv__srv__ik_service

            from rrbot_gazebo.srv import _ik_service
            if _ik_service.Metaclass_IKService_Request._TYPE_SUPPORT is None:
                _ik_service.Metaclass_IKService_Request.__import_type_support__()
            if _ik_service.Metaclass_IKService_Response._TYPE_SUPPORT is None:
                _ik_service.Metaclass_IKService_Response.__import_type_support__()


class IKService(metaclass=Metaclass_IKService):
    from rrbot_gazebo.srv._ik_service import IKService_Request as Request
    from rrbot_gazebo.srv._ik_service import IKService_Response as Response

    def __init__(self):
        raise NotImplementedError('Service classes can not be instantiated')
